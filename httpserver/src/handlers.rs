...
impl Handler for StaticPageHandler {
    fn handle(req: &HttpRequest) -> HttpResponse {
        if let http::httprequest::Resource::Path(s) = &req.resource {
            ...
            match route.get(1).copied().unwrap_or("") {
                ...
                "health" => {
                    return HttpResponse::new("200", None, Self::load_file("health.html"));
                }
                path => match Self::load_file(path) {
                    Some(contents) => {
                        ...
                        return HttpResponse::new("200", Some(map), Some(contents));
                    }
                    None => {
